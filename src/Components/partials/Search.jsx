...
const Search = () => {
      ...
      return (
            <>
                  <section className={`w-full min-h-[100dvh] z-20 ${style.scrollbar} sm:mx-auto  bg-[rgb(31,71,69)] bg-[linear-gradient(27deg,_rgba(31,71,69,1)_10%,_rgba(16,36,27,1)_67%,_rgba(68,73,53,1)_100%)] backdrop-blur-xl  px-5 overflow-y-scroll text-white`}>
                        <motion.div initial={{ y: -100 }} animate={{ y: 0, transition: { ease: "backInOut", duration: 0.5 } }} className={`relative ${style.scrollbar} mt-5 w-full`}>
                              <input value={query} onChange={(e) => setQuery(e.target.value)} autoFocus className="w-full text-[#F5CD80] pr-12 pl-14 outline-none font-Stoshi border-none rounded-full bg-black/40 caret-[#d0ab67]  selection:bg-[#F5CD80] selection:text-black backdrop-blur-sm md:text-xl  py-3" placeholder="Search for movies, tv shows, people" type="text" />
                              ...
                              {movieData &&
                                    movieData.map((eachMovie, index) => (
                                          <Link to={`/${eachMovie.media_type}/details/${eachMovie.id}`} key={index} className="w-full px-5 flex  items-center gap-5  mt-3">
                                                <div className="w-20 md:w-28 md:h-28 shrink-0 h-20 rounded-xl overflow-hidden">
                                                ...
                                                <h1 className="leading-none md:text-xl">{eachMovie.name || eachMovie.original_name || eachMovie.original_title}</h1>
                                          </Link>
                                    ))}
                        </div>
                  </section>
